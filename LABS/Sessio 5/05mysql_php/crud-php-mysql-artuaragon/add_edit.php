...
if(isset($_POST['add']) or isset($_POST['update'])) {	
	...
	if(empty($username) or empty($passwd)) {
        ...
        if(isset($_POST['add'])) {
            ...
            // AIXO PERMET SQL INJECTION: $result = mysqli_query($mysqli, "INSERT INTO users(username,passwd) VALUES('$username','$passwd')");
            
            // AIXO NO:
            
			$result = $mysqli->prepare("INSERT INTO users (username, passwd) VALUES(?,?)");

			$result->bind_param("ss",$_POST['username'],$_POST['passwd']);
        ...
        if(isset($_POST['update'])) {
    		//updating the table
    		// AIXO PERMET SQL INJECTION: $result = mysqli_query($mysqli, "UPDATE users SET passwd='$passwd' WHERE username='$username'");

			// AIXO NO:

			$result = $mysqli->prepare("UPDATE users SET passwd=? WHERE username=?");

			$result->bind_param("ss", $_POST['passwd'], $_POST['username']);
